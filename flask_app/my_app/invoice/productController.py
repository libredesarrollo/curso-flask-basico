...
from flask import Blueprint, render_template, request, redirect, url_for, flash, get_flashed_messages,jsonify, send_file
from flask_login import login_required
from flask_user import roles_required

invoiceProductBp = Blueprint('invoice_product',__name__, url_prefix='/invoice')

@invoiceProductBp.before_request
def constructor():
   pass

@invoiceProductBp.route('detail/<int:id>')
def detail(id): 
...
@invoiceProductBp.route('detail_pdf/<int:id>')
def detail_pdf(id): 
...
@login_required
@roles_required('Regular')
@invoiceProductBp.route('/')
@invoiceProductBp.route('/<int:page>')
def index(page=1):
...
@login_required
@roles_required('Regular')
@invoiceProductBp.route('/dispatch')
def dispatch():
...
@invoiceProductBp.route('jsearch_product')
def jsearch_product():
...
@login_required
@roles_required('Regular')
@invoiceProductBp.route('jsell', methods=['POST'])
def jsell():
